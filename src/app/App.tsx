...
const SIDEBAR_WIDTH_STORAGE_KEY = "flame-code.sidebar.width";
const SIDEBAR_VIEW_STORAGE_KEY = "flame-code.sidebar.view";
      ...
      (event) => {
        ...
        void (async () => {
          try {
            ...
            if (!parsed.ok) {
              console.warn("[flame-code] theme not applied:", parsed.error);
              return;
            }
            await saveCustomTheme(parsed.theme);
          } catch (e) {
            console.warn("[flame-code] theme ingest failed:", e);
          }
        })();
      },
    );
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("flame-code:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
      ...
      spawnManagedAgent: (prompt: string, sessionId: string) => {
        ...
        void (async () => {
          ...
          if (result !== "ready") {
            if (result === "timeout") {
              console.warn(
                "[flame-code] Claude TUI did not appear in time; aborting prompt send",
              );
            }
            useManagedAgentsStore.getState().remove(leafId);
            return;
