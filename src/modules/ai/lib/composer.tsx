...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("flame-code:ai-attach-file", onAttach);
    return () => window.removeEventListener("flame-code:ai-attach-file", onAttach);
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<fl-code-command name="${outcome.commandName}" />`;
        }
      }
    }
