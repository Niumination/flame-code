...
const OWNED_MARKERS: [&str; 3] = ["notify;Flame Code;", "flame-code;notify", "terax;notify"];

// Gated on FLAME_CODE_TERMINAL; no-op outside Flame Code. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$FLAME_CODE_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Flame Code;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.flame-code-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;Flame Code;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Flame Code;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Flame Code;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Flame Code;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$FLAME_CODE_TERMINAL\" ] && printf '\\033]777;terax;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
